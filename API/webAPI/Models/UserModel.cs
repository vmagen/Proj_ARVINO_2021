...
namespace webAPI.Models
{
    public class UserModel
    {
        ...
        public static void SetPrimaryUsers()
        {
            try
            {
                using (var db = new ArvinoDbContext())
                {
                    ...
                    var list = db.RV_KNNCategory.ToList();
                    foreach (var item in list)
                    {
                        int? sum = 0;
                        sum += item.red + item.white + item.rose + item.bubble + item.Galil + item.Golan + item.Carmel + item.negev + item.shomron + item.yehuda;
                        var singleUser = db.RV_User.SingleOrDefault(i => i.email == item.email);
                        if (singleUser != null && sum > 100)
                        {
                            singleUser.isPremium = true;
                            db.SaveChanges();
                        }
                    }
                }
